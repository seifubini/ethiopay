...
$(document).ready(function() {
    var ticketTable = $('#ticketDatatable').DataTable({
            ...
            "ajax": {
                "url": "{{ url('admin/tickets/datatable') }}",
                "data": function (d) {
                }
            },
            columns: [
                ...
                },                
                {
                    ...
                    "render":function(o){
                        var str="";
                        str += "<a href='{{ url('admin/tickets') }}/" + o.id +"' class='viewRecord' val='" + o.ticket_id + "'><i class='glyphicon glyphicon-eye-open'></i></a> ";
                        return str;
                    }
                }
            ],
            ...
            swal({
                ...
                preConfirm: function (email) {
                    return new Promise(function (resolve, reject) {
                        setTimeout(function() {
                            jQuery.ajax({
                                url: "{{ url('admin/change-ticket-status') }}/" + id ,
                                type: 'GET',
                                dataType: 'json',
                                data: {
                                    "status": status
    ...
    {{--  jQuery(document).on('change', '.statusChange', function(event) {
        var id = $(this).data('id');
        $.ajax({
            url: "{{ url('admin/change_ticket_status') }}/" + id ,
